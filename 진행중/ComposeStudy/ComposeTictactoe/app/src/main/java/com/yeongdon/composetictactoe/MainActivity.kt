package com.yeongdon.composetictactoe
...
import com.yeongdon.composetictactoe.ui.theme.ComposeTictactoeTheme
...
fun TTTScreen() {
    ...
    val onTap: (Offset) -> Unit = {
        if (playerTurn.value && win.value == null) {
            ...
            val posIntMoves = y * 3 + x
            if (moves[posIntMoves] == null) {
                moves[posIntMoves] = true
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        if (win.value != null) {
            when (win.value) {
                Win.PLAYER -> {
                    Text(text = "Player 승리 \uD83C\uDF89", fontSize = 25.sp)
                }

                Win.AI -> {
                    Text(text = "AI 승리 \uD83D\uDE24", fontSize = 25.sp)
                }

                Win.DRAW -> {
                    Text(text = "무승부 \uD83D\uDE33", fontSize = 25.sp)
                }
                else -> {}
            }
            Button(onClick = {
